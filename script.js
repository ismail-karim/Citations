...
let btnNew = document.getElementById('nouveau');

let dernier = 0;
let nbAlea = 0;
function genereNbAlea(max){
    return Math.floor(Math.random()*Math.floor(max));
}
btnNew.addEventListener('click', () =>{
    do{
        nbAlea = genereNbAlea(citations.length);
    }while (nbAlea === dernier)
    citation.textContent = citations[nbAlea][0];
    auteur.textContent = citations[nbAlea][1];
    dernier = nbAlea;
});